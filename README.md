# PROJECT_PICO8
This is a PICO-8 game project in progress on and off. I chose this very loose, lightweight, assembley-like engine to challenge my self with building systems and features of a game engine from scratch including collision detection and filtering, object class inheritence and flexibility, sprite parsing and animation systems, and OOP management.
...
1. Nomenclature semantics:
   ...
   - Constants are all caps! All of these are very similar to habbits adopted in C++ and C# programming

2. Basic class system:
   - Using the LUA manual, I devised a series of ZObject classes that build off eachother with inheritance
   - Designing the classes and where things live, what variables are shared, what variables are overriden, is very difficult to determine as the project evolves and new requirements come up (despite thoughtout planning). I try to lean on computer science concepts, namely encapsulation and the one responsibility principle per function.

3. Object Manager:
  - I created an object manager to be able to start divide objects into different groups for different purposes (static objects, player object, enemeies, etc). This would be essential for collision filtering and detection
...
- Since animation data would be needed to define frame pacing, cooordinates for sprite extraction, whether an animation loops, etc., I separated that into a static animation file defining these in their own classes via animData.lua.
